...
import { FiArrowLeft, FiCalendar, FiCheckCircle, FiClock, FiDollarSign, FiEdit2, FiTruck, FiTrash2, FiUser, FiX, FiXCircle, FiAlertTriangle, FiEye } from 'react-icons/fi';
...
export default function BookingDetail() {
  ...
  const backPath = isAdmin ? '/bookings' : '/my-bookings';
  ...
  const onConfirm = () => {
    ...
    runAction(() => bookingAPI.confirm(booking._id), 'Đã xác nhận booking');
  };
  ...
  const onMarkOverdue = () => {
    if (!window.confirm('Đánh dấu đơn này là quá hạn trả xe?')) return;
    runAction(() => bookingAPI.markOverdue(booking._id), 'Đã đánh dấu quá hạn trả xe');
  };

  const onComplete = () => {
    if (!window.confirm('Xác nhận hoàn thành booking?')) return;
    runAction(() => bookingAPI.complete(booking._id), 'Hoàn thành booking');
  };
  ...
  const canPayDeposit = !isAdmin && bookingStatus === 'chờ xác nhận' && !booking.depositPaidAt;
  const canConfirm = isAdmin && bookingStatus === 'chờ xác nhận' && Boolean(booking.depositPaidAt);
  const canPayRemaining = !isAdmin && bookingStatus === 'chờ nhận xe' && !booking.remainingPaidAt;
  ...
  const canEdit = isAdmin && (bookingStatus === 'chờ xác nhận' || bookingStatus === 'chờ nhận xe');
  const canDelete = isAdmin;
  const canMarkOverdue = isAdmin && booking.overdue && (bookingStatus === 'đã nhận xe' || bookingStatus === 'quá hạn trả xe');
  const canComplete = isAdmin && (bookingStatus === 'đã nhận xe' || bookingStatus === 'quá hạn trả xe');
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-2xl font-bold flex items-center gap-2"><FiEye /> Chi tiết đặt xe</h2>
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {canConfirm && (
            <button disabled={acting} onClick={onConfirm} className="bg-indigo-600 text-white px-3 py-2 rounded-lg hover:bg-indigo-700 transition text-sm inline-flex items-center gap-2 disabled:opacity-60">
              <FiCheckCircle /> Admin xác nhận
            </button>
          )}
          {canPayRemaining && (
          ...
          {canMarkOverdue && (
            <button disabled={acting} onClick={onMarkOverdue} className="bg-rose-600 text-white px-3 py-2 rounded-lg hover:bg-rose-700 transition text-sm inline-flex items-center gap-2 disabled:opacity-60">
              <FiAlertTriangle /> Đánh dấu quá hạn
            </button>
          )}
          {canComplete && (
            <button disabled={acting} onClick={onComplete} className="bg-green-600 text-white px-3 py-2 rounded-lg hover:bg-green-700 transition text-sm inline-flex items-center gap-2 disabled:opacity-60">
              <FiCheckCircle /> Hoàn thành
            </button>
          )}
          {canEdit && (
            <Link to={`/bookings/edit/${booking._id}`} className="bg-blue-600 text-white px-3 py-2 rounded-lg hover:bg-blue-700 transition text-sm inline-flex items-center gap-2">
              <FiEdit2 /> Chỉnh sửa
            </Link>
          )}
          {canCancel && (
          ...
          {canDelete && (
            <button disabled={acting} onClick={onDelete} className="bg-red-600 text-white px-3 py-2 rounded-lg hover:bg-red-700 transition text-sm inline-flex items-center gap-2 disabled:opacity-60">
              <FiTrash2 /> Xóa đơn
            </button>
          )}
          {!canPayDeposit && !canConfirm && !canPayRemaining && !canPickup && !canMarkOverdue && !canComplete && !canEdit && !canCancel && !canDelete && (
