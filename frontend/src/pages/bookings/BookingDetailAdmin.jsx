...
import { FiArrowLeft, FiCalendar, FiCheckCircle, FiClock, FiEdit2, FiTruck, FiTrash2, FiUser, FiX, FiXCircle, FiAlertTriangle, FiEye } from 'react-icons/fi';
...
export default function BookingDetailAdmin() {
  ...
  useEffect(() => {
    if (!isAdmin) {
      // Nếu là user, redirect sang trang chi tiết user
  ...
  const canConfirm = bookingStatus === 'chờ xác nhận' && Boolean(booking.depositPaidAt);
  const canEdit = bookingStatus === 'chờ xác nhận' || bookingStatus === 'chờ nhận xe';
  const canDelete = true;
  const canMarkOverdue = booking.overdue && (bookingStatus === 'đã nhận xe' || bookingStatus === 'quá hạn trả xe');
  const canComplete = bookingStatus === 'đã nhận xe' || bookingStatus === 'quá hạn trả xe';
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {canEdit && (
            <button disabled={acting} onClick={onEdit} className="bg-blue-600 text-white px-3 py-2 rounded-lg hover:bg-blue-700 transition text-sm inline-flex items-center gap-2">
              <FiEdit2 /> Chỉnh sửa
            </button>
          )}
          {canDelete && (
          ...
          {!canConfirm && !canMarkOverdue && !canComplete && !canEdit && !canDelete && (
